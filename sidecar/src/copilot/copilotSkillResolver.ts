...
export type ResolvedSkillSelection = {
  ...
  mode: "global" | "agent-scoped";
};

type SkillResolutionArgs = {
  baseSkillDirectories: string[] | null;
  envDisabledSkills: string[] | null;
  executionContext: AgentExecutionContext | null;
};

export function resolveSkillSelection(args: SkillResolutionArgs): ResolvedSkillSelection {
  ...
  const mode: "global" | "agent-scoped" = scoped && scoped.length > 0 ? "agent-scoped" : "global";
...
function discoverScopedSkillDirectories(baseDirectories: string[], agentID: string) {
...
function discoverSkillNames(skillDirectories: string[]) {
...
function readSkillName(skillManifestPath: string, fallbackName: string) {
  try {
...
function mergeLists(lhs: string[] | null, rhs: string[] | null) {
